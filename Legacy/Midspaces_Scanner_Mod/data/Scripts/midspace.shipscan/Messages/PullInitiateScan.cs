                ...
                delegate
                {
                    ...
                    foreach (var cubeGridPart in floatingList)
                    {
                        ...
                        if (!shipGrids.Any(s => s.GridGroups.Any(g => g.EntityId == cubeGridPart.EntityId)))
                        {
                            ...
                            // Check if the is not powered or Owned in any way. (Edited out for normal scanner)
                ...
                }, delegate
            {
                switch (displayType)
                {
                    case ScanType.MissionScreen:
                        {
                            var prefix = $"Scan range: {effectiveRadius}m : {shipGrids.Count} masses detected."; // Edited out "derelict"
